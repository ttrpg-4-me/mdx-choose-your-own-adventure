...
function isScriptType(type) {
    return type == "mdxjsEsm" || type == "mdxFlowExpression"
}
...
function updatePassageScript(tokens, passage) {
    if (tokens.length == 0 || (tokens.at(-1).type != "mdxjsEsm" && tokens.at(-1).type && "mdxFlowExpression")) {
        statements.push(tokens.pop().value)
    }
}
...
function parsePassage(tokens, tree) {
    ...
    const header_name = convertStringMarkdownHeaderFormat(tokens.pop().children[0].value)
